	...
	<div class="container-md" id=wrapper style="width:80%;margin:100px auto;">
		...
		<div id=maincontents style=margin-top:15px;">
			...
			<%
				...
				int pagePerBlock=15; // 블럭당 표시할 페이지 수 
				int totalBlock=0; //페이징처리 전체 페이징 블럭 수
				int nowBlock=1; 	// 현재 페이징 블럭 수
			%>
			
			<%
				//상단의 현재 페이지 번호 경경 위한 처리
				if(request.getAttribute("nowPage")!=null)
					nowPage=
					Integer.parseInt((String)request.getAttribute("nowPage"));
				System.out.println("필욱고장");
			%>
			
			<%
				totalcount = (int)request.getAttribute("tcnt"); //전체 세시물 수 받기
				totalPage = (int)Math.ceil(totalcount/numPerPage); //전체 페이지 수 계산
				
				
				//전체 블럭수 계산
				totalBlock = (int)Math.ceil((double)totalPage/ pagePerBlock); //더블로 처리후 올림처리
				
				//현재 블럭숫자 계산
				nowBlock = (int)Math.ceil((double)nowPage/ pagePerBlock); //현재블럭 페이지 
        ...
        <form name = initFrm method=get>
			...
			<table class="table">
				<tr>
					<td style=border:0px;><span style=color:blue;><%=nowPage %></span>/ <%=nowPage %> Page</td>
					<td style=border:0px;text-align:right;>
						<button class="btn btn-secondary" onclick="init()">처음으로</button>
						<a class="btn btn-secondary" href="/Board/post.do">글쓰기</a>
					</td>
					...
					<td colspan=5 style="border-bottom:0px;">
						<nav aria-label="Page navigation example">
						  <ul class="pagination">
						    ...
						    {
						    %>
						    <li class="page-item"><!-- 현재가 10이라면 다음 블럭 없다 전체블럭 -->
						      <a class="page-link" href="javascript:block(<%=nowBlock+1%>)" aria-label="Next">
						        <span aria-hidden="true">&raquo;</span>
						      </a>
						    </li>
